...
STUDENTS = ["6710301034", "6710301043", "6710301033", "6710301017", "6710301020", "6710301019"]  # กำหนดหรือปรับค่าให้ STUDENTS
GROUP_SIZE = len(STUDENTS)  # กำหนดหรือปรับค่าให้ GROUP_SIZE
TOTAL_COUPONS = (GROUP_SIZE * 2) - 1  # กำหนดหรือปรับค่าให้ TOTAL_COUPONS

coupons_db: List[str] = [f"COUPON-{i:02d}" for i in range(1, TOTAL_COUPONS + 1)]  # ดำเนินคำสั่งของบรรทัดนี้ตามลำดับการทำงาน
...
async def claim_coupon(req: ClaimRequest):  # ประกาศฟังก์ชัน claim_coupon สำหรับรวมขั้นตอนการทำงาน
    ...
    if current_coupon_index < len(coupons_db):  # ตรวจสอบเงื่อนไขก่อนเลือกเส้นทางการทำงาน
        ...
        return {"status": "SUCCESS", "claimed_coupon": coupon, "total_owned": len(student_claims[student_id])}  # ส่งผลลัพธ์กลับไปยังผู้เรียก
